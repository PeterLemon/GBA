; Game Boy Advance 'Bare Metal' BIOS Functions Decompress BIT UnPack 1BPP demo by krom (Peter Lemon):
...
start:
  ...
  PrintString BIT1BPPTEXT, 4224, 22, 5 ; Print String: Source Address, VRAM Destination, String Length, Palette Number

  imm32 r0,BITDestWidth ; Destination Unit Bit Width Pointer Address
  mov r1,1              ; Destination Unit Bit Width
  strb r1,[r0]          ; Store Destination Unit Bit Width

  imm32 r0,DATA1BPP   ; Source
  ...
  PrintString DEST1BPPTEXT, 4570, 4, 0 ; Print String: Source Address, VRAM Destination, String Length, Palette Number

  mov r0,WRAM       ; Data Address
  imm32 r1,DATA1BPP ; Check Address
  mov r2,256        ; Count = Data Length / 4
  TestLOOPA:
  ...
  imm32 r0,BITDestWidth ; Destination Unit Bit Width Pointer Address
  mov r1,2              ; Destination Unit Bit Width
  strb r1,[r0]          ; Store Destination Unit Bit Width
  ...
  PrintString DEST2BPPTEXT, 4698, 4, 0 ; Print String: Source Address, VRAM Destination, String Length, Palette Number

  mov r0,WRAM       ; Data Address
  imm32 r1,DATA2BPP ; Check Address
  mov r2,512        ; Count = Data Length / 4
  TestLOOPC:
    ldr r3,[r0],4 ; Load Data Word, Increment Data Offset
    ldr r4,[r1],4 ; Load Check Word, Increment Check Offset
    cmp r3,r4     ; Compare Data To Check
  ...
  PrintString DEST4BPPTEXT, 4826, 4, 0 ; Print String: Source Address, VRAM Destination, String Length, Palette Number
  ...
  PrintString DEST8BPPTEXT, 4954, 4, 0 ; Print String: Source Address, VRAM Destination, String Length, Palette Number
  ...
  PrintString DATATEXT, 5072, 4, 0 ; Print String: Source Address, VRAM Destination, String Length, Palette Number
  PrintString DEST16BPPTEXT, 5082, 5, 0 ; Print String: Source Address, VRAM Destination, String Length, Palette Number
  ...
  TestLOOPI:
    ...
    ldr r4,[r1],4 ; Load Check Word, Increment Check Offset
    cmp r3,r4     ; Compare Data To Check
    bne TestFAILI ; IF(Data != Check) FAIL
    subs r2,1     ; Decrement Count
    bne TestLOOPI ; IF(Count != 0) LOOP
  ...
  lsl r4,16
  lsr r4,16
  imm16 r9,$712B ; Load Test Check
  cmp r4,r9      ; Compare Result
  bne TestFAILJ  ; IF(Check != Result) FAIL, ELSE PASS
  ...
  PrintString DATATEXT, 5200, 4, 0 ; Print String: Source Address, VRAM Destination, String Length, Palette Number
  PrintString DEST32BPPTEXT, 5210, 5, 0 ; Print String: Source Address, VRAM Destination, String Length, Palette Number
  ...
  TestLOOPK:
    ...
    ldr r4,[r1],4 ; Load Check Word, Increment Check Offset
    cmp r3,r4     ; Compare Data To Check
    bne TestFAILK ; IF(Data != Check) FAIL
    subs r2,1     ; Decrement Count
    bne TestLOOPK ; IF(Count != 0) LOOP
  ...
  lsl r4,16
  lsr r4,16
  imm16 r9,$E12B ; Load Test Check
  cmp r4,r9      ; Compare Result
  bne TestFAILL  ; IF(Check != Result) FAIL, ELSE PASS
...
BITPointer:
  dh 1024 ; Source Data Byte Length
  db 1    ; Source Unit Bit Width (1,2,4,8)
BITDestWidth:
  db 1    ; Destination Unit Bit Width (1,2,4,8,16,32)
  dw 0    ; Data Offset (Bit 0..30), Zero Data Flag (Bit 31)

DATA1BPP: file 'DATA1BPP.bin' ; Include 1BPP Data (1024 Bytes)
...
BIT1BPPTEXT:   db "BIT $10 (UnPack 1BPP):"         ; Include BG Map Text Data (22 Bytes)
...
DEST1BPPTEXT:  db "1BPP"     ; Include BG Map Text Data (4 Bytes)
DEST2BPPTEXT:  db "2BPP"     ; Include BG Map Text Data (4 Bytes)
