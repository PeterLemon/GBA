; Game Boy Advance 'Bare Metal' BIOS Functions Sound MidiKey2Freq demo by krom (Peter Lemon):
...
start:
  ...
  PrintString MidiKey2FreqTEXT, 4224, 17, 5 ; Print String: Source Address, VRAM Destination, String Length, Palette Number

  imm32 r0,WAVEDATA ; WaveData
  mov r1,48         ; MIDI Key (C3)
  mov r2,0          ; Fine Adjustment
  ...
  imm32 r9,$00000010 ; Load Test Check
...
WAVEDATA: ; WaveData Structure
  dh $0000     ; Type: Indicates Data Type (Not Used)
  dh $0000     ; Stat: $0000 = Non-Looped (1 Shot) Waveform, $4000 = Forward Loop Waveform
  dw $00008000 ; Freq: Frequency
  dw $00000000 ; Loop: Loop Pointer (Start Of Loop)
  dw $00000010 ; Size: Number Of Samples (End Position)
  db $7F       ; Data: Waveform Data (Number of Samples+1) Bytes Of 8-Bit Signed Linear Uncompressed Data
  db $7F       ;       Last Byte Is Zero For Non-Looped Waveform, & Same Value As Loop Pointer Data For Looped Waveform
  db $7F
  db $7F
  db $7F
  db $7F
...
MidiKey2FreqTEXT: db "MidiKey2Freq $1F:"              ; Include BG Map Text Data (17 Bytes)
